...
def transcribe_audio_chunks(audio_chunks,api_key, language):
    try:
...
def main():
    ...
    if not webrtc_ctx.state.playing and len(audio_buffer) > 0:
        ...
        transcription = transcribe_audio_chunks(audio_buffer,DEEPGRAM_API_KEY,st.session_state["language_to_transcript"])
        if transcription:
